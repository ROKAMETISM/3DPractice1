...
func _ready() -> void:
	...
	control_fsm.state_updated.connect(_on_control_fsm_state_updated)
...
func _die() -> void:
	queue_free()
func _on_control_fsm_state_updated(new_states : Array[State])->void:
	state_label.text = ""
	for state: State in new_states:
